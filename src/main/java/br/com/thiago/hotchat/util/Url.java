package br.com.thiago.hotchat.util;

public class Url {

	public final static String CHANNEL_MESSAGE_USER = "/channel/user/";
	public final static String CHANNEL_MESSAGE_USER_PARAM = "/channel/user/{email}";
	public final static String CHANNEL_CHAT_CONTACTS_LIST = "/channel/listContacts";
	public final static String CHANNEL_MESSAGES_OFFLINE = "/channel/messagesOffline/";

	public final static String CHAT_CONTACTS_LIST = "/chat.listContacts";
	public final static String SEND_MESSAGE = "/chat.sendMessage";

}
